...
""",
    ...
    assert any(
        ...
        and fact.symbol == "ProjectionStore"
...
""",
    ...
    assert any(fact.symbol == "ProjectionStore" for fact in artifact_facts)
    assert len(alignment_records) == 1
...
""",
    ...
    assert {fact.symbol for fact in class_facts} == {"ProjectionStore"}
...
def test_import_relationship_observation_pipeline():
    source_text = "from fixtures.example import ExampleComponent\n"
    ...
    assert relationship_facts[0].object == "ExampleComponent"
...
from fixtures.example import ExampleComponent
"""
