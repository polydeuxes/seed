...
def test_filtering_works():
    ...
    predicate_inv = build_observation_inventory(predicate_filter="hostname")
    assert [p.predicate for p in predicate_inv.predicates] == ["hostname"]
    assert all(p.predicates == ("hostname",) for p in predicate_inv.providers)
