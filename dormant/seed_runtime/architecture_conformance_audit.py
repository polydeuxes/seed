...
class OperationalEvidence:
    ...
    detail: str
    confidence: str
    graph_classification: str | None = None
    ...
    def to_json_dict(self) -> dict[str, str]:
        ...
        if self.graph_classification is not None:
            result["graph_classification"] = self.graph_classification
        if self.graph_node_type is not None:
            result["graph_node_type"] = self.graph_node_type
        if self.graph_taxonomy is not None:
            result["graph_taxonomy"] = self.graph_taxonomy
        return result
    ...
    def to_json_dict(self) -> dict[str, Any]:
        return {
            ...
            "architecture_evidence": [
                e.to_json_dict() for e in self.architecture_evidence
            ],
            "operational_evidence": [
                e.to_json_dict() for e in self.operational_evidence
            ],
            "reason": self.reason,
        }
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "classifications": dict(
                sorted(Counter(f.classification for f in self.findings).items())
            ),
            "significance": dict(
                sorted(Counter(f.significance for f in self.findings).items())
            ),
            ...
            "schema_detail_findings": sum(
                ...
                for f in self.findings
                if f.significance
                in {"schema_detail", "observation_detail", "leaf_node"}
            ),
            "concept_realizations": len(self.concept_realizations),
            "realization_assessments": dict(
                sorted(Counter(r.assessment for r in self.concept_realizations).items())
            ),
            **self.metadata,
        }
    ...
    def to_json_dict(self) -> dict[str, Any]:
        return {
            ...
            "concept_realizations": [
                r.to_json_dict() for r in self.concept_realizations
            ],
            "metadata": dict(self.metadata),
        }


def build_architecture_conformance_audit(
    root: str | Path | None = None,
    *,
    architecture_evidence: tuple[ArchitectureEvidence, ...] | None = None,
    graph: OperationalGraph | None = None,
) -> ArchitectureConformanceAudit:
    repo_root = Path(root) if root is not None else Path(__file__).resolve().parents[1]
    arch = (
        architecture_evidence
        if architecture_evidence is not None
        else discover_architecture_evidence(repo_root)
    )
    ...
    return ArchitectureConformanceAudit(
        findings,
        realizations,
        {
            "read_only": True,
            ...
            "records_diagnostics": False,
        },
    )


def architecture_conformance_audit_json(
    audit: ArchitectureConformanceAudit,
) -> dict[str, Any]:
            ...
            if term in lower:
                evidence.append(
                    ArchitectureEvidence(
                        _normalize(term),
                        rel,
                        f"architecture reference mentions {term!r}",
                    )
                )
    return tuple(
        sorted(
            {(e.subject, e.source, e.detail): e for e in evidence}.values(),
            key=lambda e: (e.subject, e.source),
        )
    )
...
def format_architecture_conformance_audit(audit: ArchitectureConformanceAudit) -> str:
    ...
    lines.extend(
        [f"  {k}: {v}" for k, v in significance.items()] if significance else ["  none"]
    )
    lines.extend(
        [
            "Significance groups:",
    ...
    if audit.concept_realizations:
        ...
        for item in audit.concept_realizations:
            ...
            lines.extend(
                [
                    f"  {e.subject} — {e.source}: {e.detail} ({e.confidence})"
                    for e in item.realizations
                ]
                or ["  none"]
            )
    ...
    for f in audit.findings:
        ...
        lines.extend(
            [f"  {e.source}: {e.detail}" for e in f.architecture_evidence] or ["  none"]
        )
        lines.append("Operational evidence:")
        lines.extend(
            [
                f"  {e.source}: {e.detail} ({e.confidence})"
                for e in f.operational_evidence
            ]
            or ["  none"]
        )
    return "\n".join(lines)
...
def _operational_evidence(graph: OperationalGraph) -> tuple[OperationalEvidence, ...]:
    ...
    for node in graph.nodes:
        ...
        confidence = "medium" if node.aggregate or _is_placeholder(subject) else "high"
        items.append(
            OperationalEvidence(
                subject,
                "operational_graph",
                f"node {node.id} classified as {node.classification}",
                confidence,
                graph_classification=node.classification,
                graph_node_type=node.type,
                graph_taxonomy="aggregate" if node.aggregate else "concrete",
            )
        )
    ...
    for edge_type, count in edge_counts.items():
        items.append(
            OperationalEvidence(
                ...
                "operational_graph",
                f"{count} {edge_type!r} relationships observed",
                "high",
            )
        )
    return tuple(items)
...
def _concept_realizations(
    arch: tuple[ArchitectureEvidence, ...], operational: tuple[OperationalEvidence, ...]
) -> tuple[ConceptRealization, ...]:
    concepts = sorted({e.subject for e in arch})
    return tuple(
        _concept_realization(
            concept, tuple(e for e in operational if _realizes(concept, e))
        )
        for concept in concepts
    )


def _concept_realization(
    concept: str, matches: tuple[OperationalEvidence, ...]
) -> ConceptRealization:
    if not matches:
        return ConceptRealization(
            concept,
            "not_observed",
            (),
            (),
            "No operational structure was found with direct or lexical evidence for this architecture concept.",
        )
    ...
    if direct and all(e.confidence != "low" for e in direct):
        return ConceptRealization(
            ...
            direct,
            _support(direct),
            "Operational vocabulary contains the architecture concept directly.",
        )
    if all(e.confidence == "low" for e in matches):
        return ConceptRealization(
            ...
            matches,
            _support(matches),
            "Only low-confidence operational evidence is available for this architecture concept.",
        )
    full_token = tuple(
        e
        for e in matches
        if _concept_tokens(concept) <= _concept_tokens(e.subject + " " + e.detail)
    )
    if full_token:
        return ConceptRealization(
            concept,
            ...
            full_token,
            _support(full_token),
            "The architecture term is absent as a node, but implementation-backed operational structures carry the same concept vocabulary.",
        )
    return ConceptRealization(
        concept,
        "partially_realized",
        matches,
        _support(matches),
        "Operational evidence overlaps the architecture concept, but does not cover the full concept vocabulary.",
    )
...
def _finding(
    subject: str,
    arch: tuple[ArchitectureEvidence, ...],
    operational: tuple[OperationalEvidence, ...],
    ...
    if arch and operational:
        if any(e.confidence == "low" for e in operational):
            return ArchitectureConformanceFinding(
                ...
                arch,
                operational,
                "Architecture and operation are both present, but operational evidence is low-confidence or indirect.",
            )
        if _conflict(subject, arch, operational):
            return ArchitectureConformanceFinding(
                ...
                arch,
                operational,
                "Architecture and operational evidence use conflicting relationship vocabulary for this subject.",
            )
        return ArchitectureConformanceFinding(
            subject,
            "aligned",
            ...
            arch,
            operational,
            "Architecture and operational graph both contain evidence for this subject.",
        )
    if operational:
        classification: Classification = (
            "emergent_structure" if len(operational) > 1 else "underspecified"
        )
        reason = _missing_architecture_reason(classification, significance)
        return ArchitectureConformanceFinding(
            subject, classification, significance, arch, operational, reason
        )
    return ArchitectureConformanceFinding(
        subject,
        "obsolete_design",
        ...
        arch,
        operational,
        "Architecture describes this subject, but the operational graph did not observe it.",
    )


def _missing_architecture_reason(
    classification: Classification, significance: Significance
) -> str:
    ...
    if classification == "emergent_structure":
        return (
            "Operational graph shows recurring structure without architecture evidence."
        )
...
def _significance(
    ...
    if _is_placeholder(subject):
        return "unknown_structure"
    graph_classes = {
        e.graph_classification for e in operational if e.graph_classification
    }
    graph_types = {e.graph_node_type for e in operational if e.graph_node_type}
    ...
    if any(
        cls in graph_classes for cls in {"concrete_diagnostic", "aggregate_diagnostic"}
    ):
        return "visibility_structure"
    if graph_types & {"diagnostic"}:
        return "visibility_structure"
    if any(
        cls in graph_classes
        for cls in {
            "concrete_emitter",
            "aggregate_emitter",
            "concrete_event",
        ...
        return "workflow_structure"
    if graph_types & {"emitter", "event"}:
        return "workflow_structure"
    if any(cls in graph_classes for cls in {"concrete_surface", "aggregate_surface"}):
        return "visibility_structure"
    if graph_types & {"surface"}:
        return "visibility_structure"
    if any(
        cls in graph_classes for cls in {"concrete_projection", "aggregate_projection"}
    ):
        return "operational_structure"
    op_text = " ".join(e.detail.lower() for e in operational)
    if (
        "observation_predicate" in op_text
        or "observation predicate" in op_text
        or "observation_predicate" in graph_types
    ):
        return "observation_detail"
    if any(
        token in subject
        for token in (" uid", " count", " version", " options", " method")
    ):
        return "schema_detail"
    if arch and operational:
        return "operational_structure"
...
def _finding_sort_key(finding: ArchitectureConformanceFinding) -> tuple[int, str, str]:
    rank = {
        ...
        "unknown": 5,
        "schema_detail": 6,
        "observation_detail": 7,
        "leaf_node": 8,
    }
    return (rank[finding.significance], finding.classification, finding.subject)


def _is_placeholder(subject: str) -> bool:
    tokens = set(re.findall(r"[a-z0-9]+", _normalize(subject)))
    return bool(tokens & {"unknown", "placeholder", "unresolved", "synthetic"})
...
def _conflict(
    ...
    op_text = " ".join(e.detail.lower() for e in operational)
    return (
        subject in {"projection", "view"}
        and "relationship" in op_text
        and "view" in arch_text
    )
