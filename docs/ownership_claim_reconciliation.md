...
| `ContextComposer owns decision-context composition.` | Ownership, with composition behavior prerequisite | Context composition behavior may show ContextComposer builds decision context. Ownership requires evidence that it is the authoritative holder for composition semantics rather than one participant among budgeters, selection helpers, state views, evidence views, or runtime envelope builders. |
