# Competency Interrogation Slice 135
...
```text
DiagnosticSurface Definition Line-Set Shape-Registration-Status Inclusion Request
        !=
DiagnosticSurface Definition Shape-Registration-Status Line Rendering
```

This slice begins immediately adjacent to Slice 134 in the DiagnosticSurface definition line-set assembly path in `seed_runtime/diagnostic_inventory.py`. After the definition line-set inventory-registration inclusion request became observable, the neighboring implementation-local responsibility directly supported by the same implementation is the definition line-set assembly request that includes the already-rendered shape-registration-status line. The definition line-set assembler owns ordered inclusion in the human-rendering line set, while the shape-registration-status line renderer owns producing the concrete field line artifact.
...
- `_assemble_diagnostic_surface_definition_line_set(...)` prepares the definition shape-registration-status value and field label before returning `_DiagnosticSurfaceDefinitionLineSet`.
- `_render_diagnostic_surface_definition_shape_registration_status_line(...)` already exists as the producer for the definition shape-registration-status line request.
...
- The line-set assembly implementation calls `_render_diagnostic_surface_definition_shape_registration_status_line(...)` for the shape-registration-status row rather than constructing the row inline.
- The definition line-set assembly test now proves that the shape-registration-status row appears in the assembled line set and that line-set assembly delegates shape-registration-status row production to the definition shape-registration-status line renderer.
...
The definition line-set assembly test proved the line-set artifact, heading placement, CLI flag inclusion, implementation-reason inclusion, evidence-source inclusion, diagnostic-surface consumption inclusion, diagnostic-inventory-registration inclusion, dataclass shape, and downstream formatting join. It did not prove that shape-registration-status inclusion in the line set remained separated from shape-registration-status line rendering.
...
`test_diagnostic_surface_definition_line_set_assembly_precedes_human_rendering` now proves that `_assemble_diagnostic_surface_definition_line_set(...)` includes the rendered shape-registration-status row and delegates that row's production to `_render_diagnostic_surface_definition_shape_registration_status_line(...)`.

`_render_diagnostic_surface_definition_shape_registration_status_line(...)` remains the producer for the definition shape-registration-status line. `_assemble_diagnostic_surface_definition_line_set(...)` remains responsible only for line-set assembly and inclusion order.
...
`_assemble_diagnostic_surface_definition_line_set(...)` is the recovered producer for DiagnosticSurface definition line-set shape-registration-status inclusion.
...
The helper carrying the boundary is `_assemble_diagnostic_surface_definition_line_set(...)`, which consumes the field-specific shape-registration-status line renderer rather than owning shape-registration-status line text construction.

It does not carry shape-registration-status value preparation authority, shape-registration-status field-label production authority, shape-registration-status line-rendering authority, event-ledger authority, cluster mutation authority, schema authority, public CLI authority, or generalized rendering authority.
...
DiagnosticSurface definition line-set shape-registration-status inclusion and DiagnosticSurface definition shape-registration-status line rendering were previously compressed in the definition line-set assembly test evidence because the test did not prove that the shape-registration-status row is included through the field-specific line renderer.
...
`_assemble_diagnostic_surface_definition_line_set(...)` owns the recovered line-set shape-registration-status inclusion responsibility.
...
- `competency_interrogation_slice_135.md`
...
- broader diagnostic identity, capability, responsibility, inquiry-boundary, explanation, definition, or rendering semantics outside this local shape-registration-status line-set inclusion path.
