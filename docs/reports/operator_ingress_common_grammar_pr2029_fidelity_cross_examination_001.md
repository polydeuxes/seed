...
does **not** faithfully project operator initialization-era through the eight dimensions.
...
* Common-grammar work is prerequisite movement before possible BOGE. BOGE
  remains operator-origin; Seed may not manufacture a Seed-origin initialization-era
  goal.
...
## 4. Eight-dimensional matrices for every initialization-era occurrence
...
| scope / locality | **E:** one workspace/session/interaction/channel occurrence. **P:** ingress adapter. **Prov:** scoped ids. **L:** exact locality. **Cur:** only that interaction. **U:** relation to other interactions absent evidence. **¬:** no global initialization-era or shared grammar. |
...
| scope / locality | **E:** one interaction, declared retry policy, time/as-of boundary, obstruction predicate. **P:** policy/projector. **Prov:** scope fields. **L:** exact locality. **Cur:** scoped currentness and recurrence only. **U:** equivalence across interactions. **¬:** recurrence is not a universal initialization-era condition. |
...
| `writes_event_ledger=false` | **incorrect application boundary** | Correct for the pure binder itself, incorrect for a production initialization-era road whose occurrences must support currentness, supersession, recurrence, and later Demand examination. Event recording still does not mutate cluster truth. |
| `mutates_projected_state=false` | **category error if read as projection prohibition** | Producers should not directly mutate projected State. The `StateProjector` must nevertheless derive a current initialization-era view by replaying recorded evidence. Projection is not producer mutation. |
...
10. Keep BOGE operator-origin and require preserved ingress to re-enter
    ...
    grammar exists.
11. Preserve `mutates_cluster=false` throughout this read-only initialization-era.
...
-> initialization-era projector derives preserved-ingress current standing
...
* `StateProjector.apply` remains the event-kind dispatch boundary. It must gain
  narrow initialization-era handlers rather than receive direct producer mutations.
...
1. A real free-form initialization-era mode/caller in `scripts/seed_local.py` with
...
4. A initialization-era projection/read model preserving the eight dimensions,
...
| --- | --- | ---: |
| `scripts/seed_local.py` | explicit real free-form initialization-era caller and narrow stdin/stdout adapter | 55–90 |
...
| `seed_runtime/operator_ingress_projection.py` | eight-dimensional initialization-era view and replay reducers/currentness rules | 140–210 |
| `seed_runtime/state.py` | dispatch initialization-era event kinds into the dedicated reducer/view | 20–40 |
...
| the proposed operator-ingress CLI test path described in that snapshot | real caller, output, EOF, recording, retry policy | 130–200 |
