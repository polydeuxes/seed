# Competency Interrogation Slice 133
...
```text
DiagnosticSurface Definition Line-Set Consumption Inclusion Request
        !=
DiagnosticSurface Definition Consumption Line Rendering
```

This slice begins immediately adjacent to Slice 132 in the DiagnosticSurface definition line-set assembly path in `seed_runtime/diagnostic_inventory.py`. After the definition line-set implementation-reason inclusion request became observable, the neighboring implementation-local responsibility directly supported by the same implementation is the definition line-set assembly request that includes the already-rendered diagnostic-surface-consumption line. The definition line-set assembler owns ordered inclusion in the human-rendering line set, while the definition consumption line renderer owns producing the concrete consumption line artifact.
...
- `_assemble_diagnostic_surface_definition_line_set(...)` prepares the definition consumption text and consumption field label before returning `_DiagnosticSurfaceDefinitionLineSet`.
- `_render_diagnostic_surface_definition_consumption_line(...)` already exists as the producer for the definition consumption line request.
...
- The line-set assembly implementation calls `_render_diagnostic_surface_definition_consumption_line(...)` for the consumption row rather than constructing the row inline.
- The definition line-set assembly test now proves that the consumption row appears in the assembled line set and that line-set assembly delegates consumption row production to the definition consumption line renderer.
...
The definition line-set assembly test proved the line-set artifact, heading placement, CLI flag inclusion, implementation-reason inclusion, evidence-source inclusion, diagnostic-surface consumption row presence, dataclass shape, and downstream formatting join. It did not prove that diagnostic-surface consumption inclusion in the line set remained separated from diagnostic-surface consumption line rendering.
...
`test_diagnostic_surface_definition_line_set_assembly_precedes_human_rendering` now proves that `_assemble_diagnostic_surface_definition_line_set(...)` includes the rendered diagnostic-surface-consumption row and delegates that row's production to `_render_diagnostic_surface_definition_consumption_line(...)`.

`_render_diagnostic_surface_definition_consumption_line(...)` remains the producer for the definition diagnostic-surface-consumption line. `_assemble_diagnostic_surface_definition_line_set(...)` remains responsible only for line-set assembly and inclusion order.
...
`_assemble_diagnostic_surface_definition_line_set(...)` is the recovered producer for DiagnosticSurface definition line-set consumption inclusion.
...
The helper carrying the boundary is `_assemble_diagnostic_surface_definition_line_set(...)`, which consumes the field-specific consumption line renderer rather than owning consumption line text construction.

It does not carry consumption text preparation authority, consumption field-label production authority, consumption line-rendering authority, event-ledger authority, cluster mutation authority, schema authority, public CLI authority, or generalized rendering authority.
...
DiagnosticSurface definition line-set diagnostic-surface-consumption inclusion and DiagnosticSurface definition diagnostic-surface-consumption line rendering were previously compressed in the definition line-set assembly test evidence because the test asserted the consumption row's presence without proving that the row is included through the field-specific line renderer.
...
`_assemble_diagnostic_surface_definition_line_set(...)` owns the recovered line-set consumption inclusion responsibility.
...
- `competency_interrogation_slice_133.md`
...
- broader diagnostic identity, capability, responsibility, inquiry-boundary, explanation, definition, or rendering semantics outside this local consumption line-set inclusion path.
