...
Seed's major lifecycle documentation is stable enough to shift active architecture attention away from recursive audits and toward bounded Knowledge Acquisition expansion.
...
| --- | --- | --- |
| Knowledge Acquisition | **Implemented / Partially Complete / Architecturally Stable** | The Observation → Evidence → Fact → Projection pipeline is established. Remaining value is concrete capability growth through bounded local observation slices. |
...
The strongest current candidates are bounded, read-only observation slices that add new evidence-backed facts without changing Runtime, ToolExecutor, EventLedger ownership, ProjectionStore ownership, providers, response behavior, selection behavior, integrity semantics, or policy behavior.
...
- emit bounded evidence-backed facts;
...
Maintenance should reduce duplicate authority and keep completed findings discoverable. It should not create new document categories, generated documentation, registries, inventories, or additional architecture systems.

Current maintenance should follow `documentation_authority_reconciliation.md` and `documentation_boundary_enforcement_reconciliation.md`.
...
- projection mutation, event appends, truth arbitration, provider calls, execution, verification, refresh, repair, planning, or workflow orchestration from this documentation work.
