...
| Decision parse failure | `model.decision.parse_failed` | No direct projection branch | Attempt number, parse error, optional failure classification | Retry prompt object except fields copied into failure/retry generation | Yes for parse failure; no for complete retry context beyond stored failure payload | Runtime catches `DecisionParseError` and appends `model.decision.parse_failed` | Final invalid response after exhausted parse retry is returned but not itself appended |
