...
- **Runtime path using it:** `DecisionValidator` uses the service for `call_tool` decision validation, and `ToolExecutor` injects the same service into `ToolExecutionPolicyService` before execution. [`seed_runtime/decisions.py:34-40`](../seed_runtime/decisions.py#L34-L40) [`seed_runtime/decisions.py:79-85`](../seed_runtime/decisions.py#L79-L85) [`seed_runtime/execution.py:56-71`](../seed_runtime/execution.py#L56-L71)
...
**Entry point and responsibilities.** `Runtime` is constructed with an `EventLedger`, `StateProjector`, `DecisionInputComposer`, `DecisionValidator`, `ToolExecutor`, `ToolNeedService`, and a `DecisionProducer`; it also creates its own `RecommendationRanker`, `ToolIntentGuard`, and `StatePatchService` and loads a `CapabilityCatalog` if one is not supplied. [`seed_runtime/runtime.py:37-61`](../seed_runtime/runtime.py#L37-L61) Its public entry point, `handle_user_message(workspace_id, session_id, text)`, appends `input.user_message`, projects state, composes a `DecisionInputPacket`, asks the model for a `seed_runtime.models.Decision`, validates that decision, optionally retries, and routes supported decision kinds. [`seed_runtime/runtime.py:63-166`](../seed_runtime/runtime.py#L63-L166)

**Dependencies.** The runtime path depends on the legacy compact context system (`DecisionInputComposer`/`DecisionInputPacket`), `DecisionValidator`, `ToolExecutor`, `ToolNeedService`, `CapabilityCatalog`, `RecommendationRanker`, `ToolIntentGuard`, `StatePatchService`, `StateProjector`, and `DecisionParseError`. [`seed_runtime/runtime.py:8-20`](../seed_runtime/runtime.py#L8-L20) The `DecisionProducer` protocol accepts a `DecisionInputPacket` and returns a `seed_runtime.models.Decision`; `StaticDecisionProducer` is the deterministic test producer for this interface, and legacy runtime decision/input aliases are no longer exposed. [`seed_runtime/runtime.py:23-34`](../seed_runtime/runtime.py#L23-L34)

**Decision flow.** Runtime first appends the input event, projects state, composes a context packet, then enters a retry loop bounded by `max_decision_retries`. Parse failures append `model.decision.parse_failed` and may add a retry prompt to the next context. [`seed_runtime/runtime.py:81-103`](../seed_runtime/runtime.py#L81-L103) Successfully parsed decisions append `model.decision.proposed`, are checked by `DecisionValidator`, then checked by `ToolIntentGuard` before routing. [`seed_runtime/runtime.py:105-121`](../seed_runtime/runtime.py#L105-L121) Invalid schema/state decisions append `model.decision.invalid` and may retry with `validation_errors`; intent rejections append `model.decision.intent_rejected` and may retry with `intent_errors`. [`seed_runtime/runtime.py:123-160`](../seed_runtime/runtime.py#L123-L160) If retries are exhausted, Runtime returns `RuntimeResponse(kind="invalid_decision")`. [`seed_runtime/runtime.py:162-166`](../seed_runtime/runtime.py#L162-L166)
...
| Runtime | `tests/test_tool_intent.py` | tool-intent tests | Instantiates Runtime to verify tool calls are accepted or rejected based on user intent and visible operations/tools. [`tests/test_tool_intent.py:7-19`](../tests/test_tool_intent.py#L7-L19) [`tests/test_tool_intent.py:41-101`](../tests/test_tool_intent.py#L41-L101) |
...
| Runtime provider interface | `seed_runtime/evaluations.py` | import of `DecisionProducer` | Evaluation helpers depend on the `DecisionProducer` protocol from Runtime. [`seed_runtime/evaluations.py:1-23`](../seed_runtime/evaluations.py#L1-L23) |
...
`DecisionInputComposer` owns a `ToolRegistry` and `ContextBudget`, orders goals/entities/facts/evidence/ToolNeeds, selects budgeted sections, includes selected evidence under facts, emits visible operations/tools with schemas/policy/risk, includes open ToolNeeds / capability gaps, and declares decision schema kinds `answer`, `ask_question`, `call_tool`, `request_tool`, and `refuse`. [`seed_runtime/context.py:47-136`](../seed_runtime/context.py#L47-L136) Its producer/caller is the canonical Runtime and local app factory. [`seed_runtime/runtime.py:73-76`](../seed_runtime/runtime.py#L73-L76) [`scripts/seed_local.py:659-670`](../scripts/seed_local.py#L659-L670) Its tests are in `tests/test_context.py` and the canonical Runtime tests in `tests/test_runtime_loop.py`. [`tests/test_runtime_loop.py:10-49`](../tests/test_runtime_loop.py#L10-L49)
...
The shared domain decision kind literal includes `answer`, `ask_question`, `call_tool`, `request_tool`, `propose_action_plan`, `propose_handoff_plan`, `propose_state_patch`, and `refuse`. [`seed_runtime/models.py:42-51`](../seed_runtime/models.py#L42-L51) The `Decision` model fields are `kind`, `reason`, `answer`, `question`, `tool_name`, `tool_arguments`, `tool_need`, `action_plan`, `handoff_plan`, and `state_patch`. [`seed_runtime/models.py:205-215`](../seed_runtime/models.py#L205-L215) Runtime consumes this model through `DecisionProducer.decide`, records proposed decisions with `to_plain(decision)`, validates with `DecisionValidator`, runs `ToolIntentGuard`, and routes implemented kinds. [`seed_runtime/runtime.py:23-24`](../seed_runtime/runtime.py#L23-L24) [`seed_runtime/runtime.py:81-121`](../seed_runtime/runtime.py#L81-L121) [`seed_runtime/runtime.py:247-350`](../seed_runtime/runtime.py#L247-L350)

`DecisionValidator` supports `answer`, `ask_question`, `request_tool`, `call_tool`, `propose_state_patch`, and `refuse`; other kind values, including the literal kinds `propose_action_plan` and `propose_handoff_plan`, are reported as unsupported by the validator. [`seed_runtime/decisions.py:31-57`](../seed_runtime/decisions.py#L31-L57) It validates ToolNeeds / capability gaps for snake_case name/capability and minimum summary length, and validates tool calls against the registry or projected state plus the tool input schema. [`seed_runtime/decisions.py:59-86`](../seed_runtime/decisions.py#L59-L86)
...
| Legacy compact context packet | Present | Missing | Runtime composes `DecisionInputPacket`; RuntimeLoop composes `RuntimeContext`. [`seed_runtime/runtime.py:73-76`](../seed_runtime/runtime.py#L73-L76) [`seed_runtime/runtime_loop.py:550-573`](../seed_runtime/runtime_loop.py#L550-L573) |
| Decision context view | Missing | Present | RuntimeLoop embeds `DecisionContextView`; Runtime `DecisionInputPacket` does not have a `decision_context` field. [`seed_runtime/context.py:28-42`](../seed_runtime/context.py#L28-L42) [`seed_runtime/runtime_loop.py:65-73`](../seed_runtime/runtime_loop.py#L65-L73) |
| Context budgeting | Present | Missing | `DecisionInputComposer` uses `ContextBudget.select_sections`; RuntimeLoop `_compose_context` does not budget state or decision context. [`seed_runtime/context.py:61-76`](../seed_runtime/context.py#L61-L76) [`seed_runtime/runtime_loop.py:550-573`](../seed_runtime/runtime_loop.py#L550-L573) |
| Visible tool catalog in context | Present | Present | Runtime includes schemas, policy action, and risk; RuntimeLoop includes name, summary, policy action, and risk, but not schemas. [`seed_runtime/context.py:100-110`](../seed_runtime/context.py#L100-L110) [`seed_runtime/runtime_loop.py:553-561`](../seed_runtime/runtime_loop.py#L553-L561) |
...
| Intent validation | Present | Missing | Runtime checks `ToolIntentGuard.validate(text, decision, context.tools)` and can retry; RuntimeLoop does not reference `ToolIntentGuard`. [`seed_runtime/runtime.py:113-142`](../seed_runtime/runtime.py#L113-L142) [`seed_runtime/runtime_loop.py:15-26`](../seed_runtime/runtime_loop.py#L15-L26) |
...
| Parse failure handling | Present | Missing | Runtime catches `DecisionParseError` and records parse failure metadata; RuntimeLoop provider contract returns objects directly and only handles malformed object validation. [`seed_runtime/runtime.py:81-103`](../seed_runtime/runtime.py#L81-L103) [`seed_runtime/runtime_loop.py:575-620`](../seed_runtime/runtime_loop.py#L575-L620) |
...
| `tests/test_tool_intent.py` | Runtime `ToolIntentGuard` acceptance/rejection and visible-tool validation. [`tests/test_tool_intent.py:41-101`](../tests/test_tool_intent.py#L41-L101) |
...
Runtime behaviors observed in code with limited or no direct Runtime-route test coverage include `response.refusal` is covered, but `state.patch.rejected` through Runtime appears less directly covered than successful state patch; `propose_action_plan` and `propose_handoff_plan` are model literals but are rejected as unsupported by `DecisionValidator` and have no Runtime route tests. [`seed_runtime/models.py:42-51`](../seed_runtime/models.py#L42-L51) [`seed_runtime/decisions.py:49-57`](../seed_runtime/decisions.py#L49-L57) [`seed_runtime/runtime.py:317-329`](../seed_runtime/runtime.py#L317-L329)
...
- The `DecisionInputPacket`/`DecisionInputComposer` provider boundary, including context budgeting, selected evidence under facts, open ToolNeeds / capability gaps, operation/tool schemas, and retry prompts, is only used by Runtime. [`seed_runtime/context.py:28-136`](../seed_runtime/context.py#L28-L136) [`seed_runtime/runtime.py:73-103`](../seed_runtime/runtime.py#L73-L103)
- Runtime retry behavior for invalid decisions, parse failures, and tool-intent rejection would be lost. [`seed_runtime/runtime.py:81-166`](../seed_runtime/runtime.py#L81-L166)
- `DecisionParseError` handling and parse-failure event metadata would be lost. [`seed_runtime/runtime.py:81-103`](../seed_runtime/runtime.py#L81-L103) [`seed_runtime/runtime.py:227-245`](../seed_runtime/runtime.py#L227-L245)
- Tool-intent validation with `ToolIntentGuard` would be lost. [`seed_runtime/runtime.py:113-142`](../seed_runtime/runtime.py#L113-L142)
...
- Evaluation helpers that import the `DecisionProducer` protocol would lose that import target. [`seed_runtime/evaluations.py:1-23`](../seed_runtime/evaluations.py#L1-L23)
