...
| `call_tool` | A decision kind meaning: route a validated model decision to `ToolExecutor.execute()` using `tool_name` and `tool_arguments`. | `Runtime._route()` calls `self.tool_executor.execute(...)` only for `decision.kind == "call_tool"`; `DecisionValidator` requires `tool_name` and validates input. | `call_tool` can result in policy block, confirmation, approval, validation failure, or execution failure. Therefore it means “attempt a registered tool call through the execution boundary,” not guaranteed mutation or host command execution. |
| `request_tool` | A decision kind meaning: create or reuse a `ToolNeed` for a missing capability, rank provider recommendations, and return read-only capability resolution. | `Runtime._route()` calls `ToolNeedService.create_from_decision()`, `ToolRecommendationService.recommend_for()`, and `ToolNeedService.resolve_capability()`; `DecisionValidator` requires `tool_need.name`, `summary`, and `capability`. | `resolve_capability()` may include registered operation candidates from the registry, so `request_tool` is not only “missing executable”; it can identify existing registered operations for the same capability. |
