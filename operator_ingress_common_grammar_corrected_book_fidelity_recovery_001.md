...
| Ingress occurrence recording | attempt | decoded text or EOF plus raw/examination lineage | `operator.bootstrap.ingress_occurred` or programmatic `initial_eof_occurred`; meaning expressly Unknown | workspace/session; occurrence only | probe producer on non-EOF; projector |
...
- **Consumer available:** projection replay is available; the attempt's immediate probe production consumes the decoded ingress Event as lineage. No later same-ingress constitutional examination consumer is available on this road.
- **Consumer applicable:** Unknown for any later constitutional consumer; no applicability examination exists.
- **Actually consumed:** the initial occurrence is used to identify/lineage the probe. It is not later consumed for interpretation, acquisition, Demand, BOGE, or re-examination.
...
The attempt says “Unsupported response: exact token 1 or 2 required.” Here “required” describes membership needed by this exact binder, not a constitutional requirement to retry. It returns. The consumed presentation and response remain projected; the same presentation is durably reconstructable from Events but no active retry consumer re-presents or reopens it. Original ingress is preserved but likewise unconsumed after return. The console captures the next frame as a new attempt, not as a retry: new attempt identity, new initial-capture role, no prior presentation reference, and no lineage to the prior unsupported finding.
...
4. **Initial or response representation failure:** `_capture_representation` first records raw bytes and the exact decoder outcome. The attempt then records a separate stopping Event sourced from that examination, closes only the attempt, writes a representation-insufficiency message, and returns. Initial failure occurs before an ingress occurrence; response failure occurs after presentation but before response occurrence/binding.
...
| `presentation` | presentation Event | stdout occurrence; later changed in the projection to `consumed` when response appears | actual human receipt/meaning Unknown and not added as such | control-flow consumption marker only |
| `response` | response-captured or response-EOF Event | response occurrence; normal response changed to `consumed` at binding | unsupported branch accumulates semantic Unknowns at binding | capture/occurrence visibility only |
...
The projection mutates its own nested current snapshots—for example setting presentation/response to `consumed`—while retaining each original occurrence in `dimensional_standing`. This is projection-local bookkeeping, not proof of human uptake or constitutional consumption. `current_standing` is a dictionary name, not constitutional current standing by identity. It asserts no as-of/freshness or downstream applicability contract.
...
| `representation insufficiency` / `response representation insufficiency` | faithful implementation-local identifier | Labels strict decoder failure and does not claim uninterpretable meaning; different decoder failure outcomes remain preserved. |
...
- **Single-capture continuity proved:** monkeypatch testimony proves the outer capture is passed without recapture; durable replay proves Event/projection reconstruction. It does not prove a later constitutional consumer reopens the preserved ingress.
...
-> [constitutional prerequisite discontinuity: no exact BOGE-consumer
    examination or common-grammar-constraint finding]
```
...
2. **Consumer absent — selected treatment.** Both selections end at Event/projection/stdout; no acquisition or competent stopping consumer accepts them. These later relations are outside the bounded selection witness unless stronger behavior is claimed, so absence alone is not a defect.
3. **Consumer absent — same preserved ingress.** Events/projector retain the material, but no later re-examination consumer reopens it and no lineage connects a later act. Possible later relation, not mandatory witness scope.
4. **Disconnected implementation island — BOGE.** The production console neither supplies preserved ingress nor selected treatment to BOGE. The closed-choice adapter refuses test reconstruction and the admitted-interpretation route has unrelated lineage. A BOGE establishment is not required in this witness, but it cannot be claimed from adjacency.
5. **Standing or authority Unknown — recorded local stopping competency.** EOF/failure stopping is separately and narrowly recorded, yet the evidence does not independently establish why the producer is constitutionally competent. The negative branch correctly avoids crossing this boundary.
...
- Whether attempt-local EOF/representation-failure stopping has a constitutionally sufficient competency warrant beyond its recorded self-description.
...
The current bare production road is not one uniformly faithful or unfaithful lifecycle. Its strongest continuous road faithfully owns one real stdin capture, preserves exact boundary bytes and loss testimony, performs a bounded decoder examination without calling decoding interpretation, and records ingress meaning as Unknown. It then crosses the largest corrected-Book gap: no exact BOGE consumer attempts the declared act and no responsible common-grammar-constraint finding exists before the probe is unconditionally produced.

After that gap, the treatment-choice interaction is carefully faithful in its exact local scope. Probe formation is separate from stdout presentation; response capture is separate from meaning; exact-set binding is separate from treatment selection; affirmative selection does not acquire or authorize; negative selection does not stop; unsupported nonmembership preserves semantic Unknowns; and EOF/decoder-failure branches alone record separate attempt-local stopping occurrences. Function return never impersonates stopping.
...
Accordingly: **capture, preservation, representation evidence, presentation, response occurrence, exact binding, selection-only claims, unsupported Unknowns, and return/stopping distinctions are faithful within their exact local scopes; the prerequisite invocation crosses an unevidenced consumer-local constraint and is unfaithful as a corrected-Book prerequisite witness; all alleged downstream constitutional continuation is disconnected or Unknown rather than established.**
