...
```text
active Book
→ implementation and tests as testimony
→ reports and correction records as attributed testimony
→ history as provenance
```

The Book VII excision record (`book_vii_operational_topic_collection_excision_001.md`) is not controlling constitutional law by itself. It explicitly functions as correction testimony and as a locator to the active clauses and navigation it corrected. It is strong attributed testimony that the former operational-topic collection was excised and that external operational terms do not become Seed-native constitutional kinds merely by appearing in prose; it is not independent authority apart from the active clauses and navigation.

`book_of_seed/grammar_bounded_orientation_observation_001.md` is later attributed testimony from PR 2240. It is useful because it diagnoses recursive replacement contamination, but it does not amend active law.

Governing question, as set by curator: **which exact responsibilities preserve an act, testimony, result, or standing as retrievable material, and which current operation-related terms disappear, remain ordinary subject matter, or remain unresolved when compared to Act, production, preservation, standing, and consumer Uptake?**

All four named active files were read in full for the original recovery: `02-acts-and-constraints/acts-and-act-artifacts.md`, `05-evidence-and-knowledge/recording-and-knowledge-extraction.md`, `05-evidence-and-knowledge/testimony-and-established-fact.md`, and `06-state-and-projection/events-facts-and-state.md`. A direct case-insensitive whole-word search for `operations?` and `operational` was run across all four. `events-facts-and-state.md` contains zero occurrences of either disputed vocabulary form, but remains relevant to the Recording question because it describes event preservation.

## 2. First comparison before Responsibility decomposition

This recovery starts with bounded comparison before Responsibility decomposition. For each occurrence of `operation` or `operational`, the first question is not whether a new kind can be manufactured from the phrase. The first question is what grammatical role the phrase is trying to occupy and whether active grammar already governs that role.

The bounded distinction is:

```text
Seed-owned bounded occurrence
...
Mapping a phrase toward an established coordinate does not authorize a new compound around that coordinate.
...
PR 2240's `grammar_bounded_orientation_observation_001.md` supplies later attributed testimony for the recursive rule used here: replacing a disputed word with a phrase built around an established coordinate does not itself establish the replacement compound. In this report, that means proposed compounds such as `act-occurrence measurement`, `act-occurrence baseline`, `act-occurrence behavior`, `act-occurrence record`, and `act-occurrence testimony` are not positive mappings. They survive only as examples of defects diagnosed by PR 2240 or as rejected replacement vocabulary.

## 4. Grammar comparison by disputed phrase
...
| `operational testimony and standing` | Boundary title bundling preserved outputs | Recording/preservation can preserve attributed testimony and standing already produced or established elsewhere | Mixed | Umbrella residue only | Name the actual preserved material when known; otherwise leave unresolved |
| `operation status` / `operation-occurrence record` | PR 2238 replacement terms | Status, record, attributed testimony, measurement, or preservation grammar depending on context | Mixed | Unsupported replacement vocabulary | Unsupported; no automatic substitution with act-occurrence compounds |

No occurrence of `operation` or `operational` in the checked material survives as an independent constitutional kind. The correction is also narrower than the original report: not every occurrence maps directly to `Act` or `act occurrence`. The safe descriptions are compositional and local:

```text
measurement testimony concerning a bounded identified subject or occurrence

runtime/resource Observation testimony concerning an ambient condition

baseline standing concerning bounded ordinary behavior under its exact subject,
Evidence, scope, comparison, and standing limits

a record preserving attributed testimony that an occurrence or assertion was reported
```

These are descriptions, not new named families.

## 5. Consequence for PR 2238

PR 2238's replacements (`execution` → `operation occurrence`; `execution record` → `operation-occurrence record`; `execution status` → `operation status`) remain unsupported. The replacement vocabulary has the same defect class as the vocabulary it replaced: it appears to create an operation-centered constitutional family without exact support.

This report does not assert that any current test is broken. No test-inspection claim is made here, and no test repair is prescribed by this report.
...
the recorder
a responsible recording occurrence
```

are not enough to establish one exact instantiated responsibility with resolved:
...
occurrence Evidence
producer identity
standing-establishment identity
consumer
Uptake
```
...
The current evidence supports two live possibilities:

```text
A. Recording language compresses several responsibilities or branches.

B. The chapter already separates several responsibilities,
   but leaves some incompletely owned or instantiated.
```
...
```text
operational act:
unsupported specialization of Act where Seed-owned; external or ambient subjects
are not thereby Acts

operation / operational:
may remain ordinary subject matter; not an independent constitutional kind
...
Recording compression versus incomplete ownership:
unresolved

active amendment wording:
not established by this report
```

A later active-law amendment requires direct, recursive comparison of each operation-related occurrence and each Recording-adjacent responsibility. No replacement vocabulary or amendment scope is established by this report.
