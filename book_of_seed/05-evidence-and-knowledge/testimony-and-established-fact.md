...
Preserving a support basis does not require preserving an exhaustive enumeration of its support. An enumeration is one possible representation of it. Where an implementation represents support more compactly than an enumeration, that representation may require more than the support's identities to remain faithful — for a bounded population of occurrences, the rule by which they were selected, since a representation that cannot distinguish some of what was available from all of it does not preserve the same support. Such a requirement belongs to the representation and is not thereby part of the coordinate.
