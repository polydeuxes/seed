...
Preserving a support basis does not require preserving an enumeration of the occurrences taken. An enumeration is one representation of it. A representation more compact than an enumeration may require more than the occurrences to remain faithful, such as the rule by which they were selected, because a representation that cannot distinguish some of what was available from all of it does not preserve the same support. That requirement belongs to the representation and is not thereby part of the coordinate.
