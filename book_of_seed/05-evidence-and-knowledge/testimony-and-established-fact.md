...
Preserving a support basis does not require preserving an exhaustive enumeration of its support. An enumeration is one possible representation of it. Where an implementation represents support more compactly than an enumeration, that representation may require more than the support's identities to remain faithful, including enough to distinguish the represented support from other material within the same declared boundary. Such a requirement belongs to the representation and is not thereby part of the coordinate.
