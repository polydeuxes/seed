...
An input's support basis is which preserved material its result was drawn from, and the rule by which that material was taken. Evidence is that material; the support basis is which of it was taken and how. Preserving a support basis does not require preserving an enumeration of every supporting occurrence: an enumeration is one representation of a support basis and is not the coordinate itself.

The rule is not incidental to it. Where every act consumes a complete bounded population, which material was taken is recoverable from scope alone; where an act consumes some of that population, an account omitting the rule describes a different support than the one taken. A support basis that cannot distinguish those has not preserved the coordinate.

A premise-relative input is a different dependency. `05.Testimony:24` admits a recorded claim, finding, or evidence record consumed as premise-relative input for an exact declared act; such an input bounds what an act asked rather than supplying what its result was drawn from. A result may carry both, either, or neither, and carrying a premise establishes nothing about the support, as consuming support establishes nothing about a premise.
