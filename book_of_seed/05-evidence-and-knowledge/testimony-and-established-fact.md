...
A recorded claim, diagnostic finding, or evidence record may be consumed as attributed testimony or premise-relative input for an exact declared act. That consumption preserves attribution, source-relative limits, conflicts, and uncertainty; it does not establish the testified content as fact, prove the source producer occurred, or strengthen standing merely through repetition or copied lineage.
...
Preserving a support basis does not require preserving an exhaustive enumeration of its support. An enumeration is one possible representation of it. Where an implementation represents support more compactly than an enumeration, that representation may require more than the support's identities to remain faithful. Where the support was taken from a bounded candidate set, the representation may require the rule by which it was taken, since one that cannot distinguish some of that set from all of it does not preserve the same support. Such a requirement belongs to the representation and is not thereby part of the coordinate.
...
Trajectory establishment may be understood, where warranted, as constrained movement from occurrence history to bounded temporal understanding. Retained history is not trajectory establishment, count increased is not temporal understanding moved, and trajectory standing is not causal standing. Trajectory establishment must consume applicable series identity, temporal authority, scope, comparison method, continuity, missing testimony, conflict, evidence warrant for the bounded temporal claim, transition vocabulary, and uncertainty before it can revise condition, expectation, gap, capability, or inquiry standing.
