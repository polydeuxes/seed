# Ownership, Discrepancy, and Residue

## Constitutional subject
The characterization of current responsibility, competing ownership claims, invalid paths, naming conflicts, and historical residue.

## Core question
How should Seed distinguish lawful current ownership from implementation adjacency, stale claims, and compatibility residue?

## Initial resolution
Ownership is recovered by comparing implementation behavior, producers, consumers, declarations, and constitutional warrants. Discrepancies and residue should be projected findings, not manually frozen into this Book.

## Important distinctions
- implementation ownership != constitutional authority
- compatibility surface != current canonical path
- naming recurrence != shared responsibility

## Representative repository anchors
- `seed_runtime/ownership_discrepancies.py`
- `seed_runtime/read_model_ownership.py`
- `seed_runtime/architecture_conformance_audit.py`

## Counterexamples or failure modes
- Declaring the module nearest a type its constitutional owner.
- Preserving a legacy side path as law because tests still characterize it.

## Related chapters
- [Projection and current state](projection-and-current-state.md)
- [Constructors and production authority](../01-grammar-and-standing/constructors-and-production-authority.md)
- [Authority scope](../08-authority-communication-and-stopping/authority-scope.md)
