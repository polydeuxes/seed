# Competency Interrogation Slice 137
...
```text
DiagnosticSurface Definition Line-Set CLI Flags Inclusion Request
        !=
DiagnosticSurface Definition CLI Flags Line Rendering
```

This slice begins immediately adjacent to Slice 136 in the DiagnosticSurface definition line-set assembly path in `seed_runtime/diagnostic_inventory.py`. After the definition line-set status inclusion request became observable, the same implementation shows another still-compressed line-set inclusion responsibility: the definition line-set assembly request includes the already-rendered CLI flags line. The definition line-set assembler owns ordered inclusion in the human-rendering line set, while the CLI flags line renderer owns producing the concrete field line artifact.
...
- `_assemble_diagnostic_surface_definition_line_set(...)` prepares the definition CLI flag display before returning `_DiagnosticSurfaceDefinitionLineSet`.
- `_render_diagnostic_surface_definition_cli_flags_line(...)` already exists as the producer for the definition CLI flags line request.
...
- The line-set assembly implementation calls `_render_diagnostic_surface_definition_cli_flags_line(...)` for the CLI flags row rather than constructing the row inline.
- The definition line-set assembly test now proves that the CLI flags row appears in the assembled line set and that line-set assembly delegates CLI flags row production to the definition CLI flags line renderer.
...
The definition line-set assembly test proved the line-set artifact, heading placement, status inclusion, implementation-reason inclusion, evidence-source inclusion, diagnostic-surface consumption inclusion, diagnostic-inventory-registration inclusion, shape-registration-status inclusion, dataclass shape, and downstream formatting join. It also proved the CLI flags text appeared in the line set, but it did not prove that CLI flags inclusion in the line set remained separated from CLI flags line rendering.
...
`test_diagnostic_surface_definition_line_set_assembly_precedes_human_rendering` now proves that `_assemble_diagnostic_surface_definition_line_set(...)` includes the rendered CLI flags row and delegates that row's production to `_render_diagnostic_surface_definition_cli_flags_line(...)`.

`_render_diagnostic_surface_definition_cli_flags_line(...)` remains the producer for the definition CLI flags line. `_assemble_diagnostic_surface_definition_line_set(...)` remains responsible only for line-set assembly and inclusion order.
...
`_assemble_diagnostic_surface_definition_line_set(...)` is the recovered producer for DiagnosticSurface definition line-set CLI flags inclusion.
...
The helper carrying the boundary is `_assemble_diagnostic_surface_definition_line_set(...)`, which consumes the field-specific CLI flags line renderer rather than owning CLI flags line text construction.

It does not carry CLI flag display preparation authority, CLI flags line-rendering authority, event-ledger authority, cluster mutation authority, schema authority, public CLI authority, or generalized rendering authority.
...
DiagnosticSurface definition line-set CLI flags inclusion and DiagnosticSurface definition CLI flags line rendering were previously compressed in the definition line-set assembly test evidence because the test did not prove that the CLI flags row is included through the field-specific line renderer.
...
`_assemble_diagnostic_surface_definition_line_set(...)` owns the recovered line-set CLI flags inclusion responsibility.
...
- `competency_interrogation_slice_137.md`
