...
| terminal keystroke | `dfcfcac0` | Does one exact DEL byte change the raw result of one fixed opaque PTY/readline invocation while source bytes and process coordinates remain exact? | Partly crossed.  Each source and stdout result traverses the supplied-invocation console road in one fresh system Locality.  Each stdout Ingest preserves its exact prior source occurrence as provenance.  Every supplied occurrence receives its own append-tip byte and occurrence-position Measurement before any raw emission, so consecutive source-set Assertions preserve exactly one newly acquired occurrence.  Standing carries every result.  The distinction between the two paired results is not yet a responsible Seed finding. |
