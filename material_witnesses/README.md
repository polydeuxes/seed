...
| terminal style | `4880e4aa` | Can two exact materials produce equal visible terminal cells while only one carries presentation controls? | Not crossed.  An isolated tmux invocation preserves both its plain cell capture and its style-preserving capture as exact result material with source and external-function provenance.  No Seed Measurement identifies presentation controls, and no Representation rule applies recovered style to new material. |
