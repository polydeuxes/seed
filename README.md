...
Live read-only observation sources can collect practical host and monitoring metadata without execution, credentials, mutation, shell commands, or arbitrary PromQL. `--observe-ansible-inventory PATH` inspects raw file content before parser dispatch, treats `.ini`, `.yml`, and `.yaml` extensions only as fallback hints, and emits authoritative hostname, `ansible_host`, IP-address, alias, and group observations without invoking Ansible or connecting to hosts. `--observe-local-host` uses Python standard-library platform, disk, and read-only local file APIs to emit the local hostname's identity, OS, architecture, `/` disk totals, local network configuration, and mounted filesystem facts from `/proc/mounts`. `--observe-prometheus BASE_URL` performs HTTP `GET` requests to Prometheus's read API with a fixed allowlist of safe metric names: `up`, `node_uname_info`, `node_filesystem_avail_bytes`, and `node_filesystem_size_bytes`. Prometheus `up` samples also emit the metric's `job` label as the endpoint's durable `endpoint_role`. Use `--db` when you want the resulting observations to persist across CLI runs:
...
- `--impact ENTITY` resolves aliases and summarizes an entity's current types, identity aliases, availability, local network configuration, endpoints, groups, dependencies, dependents, conflicts, and related graph issues. Local network impact output keeps ordinary IP addresses under local network configuration instead of the aliases section, prioritizes the primary/default-route interface with addresses grouped as IPv4, IPv6 global, and IPv6 link-local, labels only default-route facts as gateways, collapses virtual/container/VPN interfaces into a count, includes a separate mounts section for `mount_point`, `filesystem_type`, `mounted_device`, and `mount_option` facts, includes a compact storage topology section for `block_device`, `partition`, storage attributes, parent relationships, and mount relationships, includes a compact listening endpoints section for `listening_endpoint` facts, and explicitly does not infer reachability, availability, filesystem health, storage health, endpoint health, service/process ownership, or application ownership from interface, mount, storage, or listener facts.
...
- `--current-facts` prints all read-only projected Fact Views; `--current-facts ENTITY PREDICATE` keeps the focused current-fact query for a subject/predicate. This remains the complete view for local network and storage topology facts, including Docker, bridge, veth, virtual, VPN interfaces, storage attributes, parent relationships, and listener endpoint/address/protocol/port facts that default `--impact` output may summarize or collapse.
...
Local host network observation is read-only and local-only: it reads Python standard-library interface data plus safe local files such as `/proc/net/route`, `/proc/net/dev`, `/proc/net/if_inet6`, `/sys/class/net/*`, `/etc/resolv.conf`, `/run/systemd/resolve/resolv.conf`, `/run/systemd/resolve/stub-resolv.conf`, `/etc/systemd/resolved.conf`, and, when present, systemd-networkd lease files under `/run/systemd/netif/leases`. Interface role classification is derived from local names and default-route evidence: the default-route interface is `primary`, `lo` is `loopback`, Docker/bridge/veth interfaces are `container`, `virbr*` is `virtual`, Tailscale/WireGuard-style interfaces are `vpn`, and unknown non-default interfaces remain `secondary`. Address assignment is only emitted when explicit evidence exists; for example, a matching systemd-networkd lease marks an IPv4 address as `dhcp`. Static assignment is not guessed from absence of DHCP evidence because it normally requires manager-specific configuration interpretation, so it remains unknown unless a future explicit read-only evidence source supports it. Resolver facts distinguish configured local stubs such as `127.0.0.53` from configured upstream nameservers when systemd-resolved files are readable; these are configuration facts only, not DNS queries, resolver health checks, or reachability assertions. Configured route does not mean gateway reachable, configured resolver does not mean DNS works, and an interface address does not mean network reachability.

Local host storage topology observation is read-only and local-only: it reads bounded evidence from `/sys/block/*`, `/sys/class/block/*`, and `/proc/partitions` to learn `block_device`, `partition`, `block_device_size_bytes`, `block_device_rotational`, `block_device_removable`, `block_device_model`, `block_device_vendor`, and `block_device_parent` facts. It does not call storage tools or manage devices. Observed block devices and partitions do not imply availability, filesystem health, storage health, repairability, backup status, redundancy, data safety, safe removal, or performance adequacy; size is not free space, rotational is not performance adequacy, and removable is not safe-to-remove status.

Local host listening-port observation is read-only and local-only: it reads bounded evidence from `/proc/net/tcp`, `/proc/net/tcp6`, `/proc/net/udp`, and `/proc/net/udp6` to learn `listening_endpoint`, `listening_protocol`, `listening_address`, and `listening_port` facts. TCP rows are projected only when local kernel state is `LISTEN`; UDP rows describe locally bound UDP endpoints with non-zero local ports. Seed does not call `ss`, `netstat`, `lsof`, `fuser`, shell commands, subprocesses, sudo, DNS, network probes, provider APIs, or Prometheus for this observation. Listening facts describe host topology only: a bound endpoint does not imply reachability, external accessibility, availability, endpoint health, service health, process ownership, service ownership, application ownership, responsiveness, active traffic, or management authority. Oversized or possibly truncated socket-table inputs are skipped rather than partially projected.
