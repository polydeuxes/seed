# Competency Interrogation Slice 139
...
```text
DiagnosticSurface Explanation Line-Set Description Inclusion Request
        !=
DiagnosticSurface Explanation Description Line Rendering
```

This slice begins immediately adjacent to Slice 138 in the DiagnosticSurface explanation human line-set assembly neighborhood in `seed_runtime/diagnostic_inventory.py`. After the DiagnosticSurface explanation line-set CLI flags inclusion request became observable, the adjacent explanation line-set assembly path shows the same recurring implementation-local responsibility: `_assemble_diagnostic_surface_explanation_line_set(...)` includes an already-rendered nested definition description line in the explanation line set, while `_render_diagnostic_surface_explanation_description_line(...)` owns the concrete description line rendering.
...
- `_assemble_diagnostic_surface_explanation_line_set(...)` extracts the nested definition, prepares description text, prepares the description field label, selects the nested definition field indent, and returns `_DiagnosticSurfaceExplanationLineSet`.
- `_render_diagnostic_surface_explanation_description_line(...)` already exists as the producer for the explanation description line request.
...
- The line-set assembly implementation calls `_render_diagnostic_surface_explanation_description_line(...)` for the nested description row rather than constructing the row inline.
- The explanation line-set assembly test now proves that the nested description row appears in the assembled line set and that line-set assembly delegates nested description row production to the explanation description line renderer.
...
The explanation line-set assembly test proved the line-set artifact, heading placement, definition section inclusion, nested CLI flags text inclusion, nested CLI flags renderer delegation, nested diagnostic-surface-consumption inclusion, dataclass shape, and downstream formatting join. It did not prove that nested description inclusion in the explanation line set remained separated from explanation description line rendering.
...
`test_diagnostic_surface_explanation_line_set_assembly_precedes_human_rendering` now proves that `_assemble_diagnostic_surface_explanation_line_set(...)` includes the rendered nested description row and delegates that row's production to `_render_diagnostic_surface_explanation_description_line(...)`.

`_render_diagnostic_surface_explanation_description_line(...)` remains the producer for the explanation description line. `_assemble_diagnostic_surface_explanation_line_set(...)` remains responsible only for explanation line-set assembly and inclusion order.
...
`_assemble_diagnostic_surface_explanation_line_set(...)` is the recovered producer for DiagnosticSurface explanation line-set description inclusion.
...
The helper carrying the boundary is `_assemble_diagnostic_surface_explanation_line_set(...)`, which consumes the field-specific explanation description line renderer rather than owning description line text construction.

It does not carry description text preparation authority, description field-label preparation authority, description line-rendering authority, event-ledger authority, cluster mutation authority, schema authority, public CLI authority, or generalized rendering authority.
...
DiagnosticSurface explanation line-set description inclusion and DiagnosticSurface explanation description line rendering were previously compressed in the explanation line-set assembly test evidence because the test did not prove that the nested description row is included through the field-specific explanation line renderer.
...
`_assemble_diagnostic_surface_explanation_line_set(...)` owns the recovered explanation line-set description inclusion responsibility.
...
- `competency_interrogation_slice_139.md`
...
- broader diagnostic identity, capability, responsibility, inquiry-boundary, explanation, definition, or rendering semantics outside this local description line-set inclusion path.
