# Competency Interrogation Slice 170
...
```text
DiagnosticSurface Definition Evidence-Source Line-Set Inclusion
        !=
DiagnosticSurface Definition Evidence-Source Line Rendering
```

This slice begins from the implementation immediately adjacent to Slice 169 in `seed_runtime/diagnostic_inventory.py` and `tests/test_diagnostic_inventory.py`. After the definition implementation-reason line-set inclusion responsibility became directly observable, the next adjacent implementation-local responsibility is the definition line-set assembly responsibility for including the already rendered evidence-source line in the top-level DiagnosticSurface definition line set.

`_assemble_diagnostic_surface_definition_line_set(...)` owns preparing the definition evidence-source value and field label, selecting the top-level definition field indent, invoking `_render_diagnostic_surface_definition_evidence_source_line(...)`, and inserting the rendered `.line` after the implementation-reason line in `_DiagnosticSurfaceDefinitionLineSet`. `_render_diagnostic_surface_definition_evidence_source_line(...)` remains responsible only for adapting the prepared evidence-source value to the generic evidence-source line renderer.
...
- `_assemble_diagnostic_surface_definition_line_set(...)` prepares `evidence_source_value` with `_prepare_diagnostic_surface_definition_evidence_source_value(definition)`.
- `_assemble_diagnostic_surface_definition_line_set(...)` prepares `evidence_source_field_label` with `_prepare_diagnostic_surface_definition_evidence_source_field_label()`.
- `_assemble_diagnostic_surface_definition_line_set(...)` selects `field_indent` with `_select_diagnostic_surface_top_level_definition_field_indent()` and passes `field_indent.text` to the definition evidence-source line renderer.
- `_assemble_diagnostic_surface_definition_line_set(...)` inserts `_render_diagnostic_surface_definition_evidence_source_line(...).line` in the returned `_DiagnosticSurfaceDefinitionLineSet` after the implementation-reason line.
- `test_diagnostic_surface_definition_line_set_assembly_includes_evidence_source_line` proves that line-set assembly consumes the definition evidence-source renderer, forwards the prepared value artifact, forwards the prepared field label and selected indent, includes the returned line in `_DiagnosticSurfaceDefinitionLineSet`, and preserves the existing placement after implementation reason.
...
The definition evidence-source line appeared correctly in human DiagnosticSurface definition output, and existing behavior was correct. However, the implementation evidence did not separately prove that line-set assembly owns only the inclusion, forwarding, and ordering responsibility for the already recovered definition evidence-source renderer.

Definition evidence-source line-set inclusion and definition evidence-source line rendering were therefore compressed in the available test evidence.

## After

`test_diagnostic_surface_definition_line_set_assembly_includes_evidence_source_line` now proves that `_assemble_diagnostic_surface_definition_line_set(...)` invokes the definition evidence-source renderer with the prepared `_DiagnosticSurfaceEvidenceSourceValue`, forwards `field_label="evidence_source"`, forwards the selected top-level definition indent, includes the returned line in `_DiagnosticSurfaceDefinitionLineSet`, and preserves the existing placement after the implementation-reason line.

`_assemble_diagnostic_surface_definition_line_set(...)` is the recovered producer for evidence-source line-set inclusion. `_render_diagnostic_surface_definition_evidence_source_line(...)` remains the definition evidence-source line renderer.
...
`_assemble_diagnostic_surface_definition_line_set(...)` is the recovered producer for DiagnosticSurface definition evidence-source line-set inclusion.
...
DiagnosticSurface definition evidence-source line-set inclusion and DiagnosticSurface definition evidence-source line rendering were previously compressed in the test evidence because the test suite did not separately prove that line-set assembly only includes, orders, and forwards inputs to the already recovered renderer.
...
`_assemble_diagnostic_surface_definition_line_set(...)` owns the recovered definition evidence-source line-set inclusion responsibility.

### 4. What artifact or helper carries the recovered boundary, if any?

`_assemble_diagnostic_surface_definition_line_set(...)` carries the recovered boundary by producing `_DiagnosticSurfaceDefinitionLineSet` with the evidence-source line included in the existing ordered human line set.
...
- `competency_interrogation_slice_170.md`
