# Competency Interrogation Slice 143
...
```text
DiagnosticSurface Explanation Line-Set Boundary Inclusion Request
        !=
DiagnosticSurface Explanation Boundary Line Rendering
```

This slice begins immediately adjacent to Slice 142 in the DiagnosticSurface explanation human line-set assembly path in `seed_runtime/diagnostic_inventory.py`. After the explanation line-set record scope inclusion request became observable, the next adjacent field in the same implementation path is `diagnostic_surface_boundary`. `_assemble_diagnostic_surface_explanation_line_set(...)` includes an already-rendered nested boundary line in the explanation line set, while `_render_diagnostic_surface_explanation_boundary_line(...)` owns concrete boundary line rendering.
...
- `_assemble_diagnostic_surface_explanation_line_set(...)` extracts the explanation boundary, prepares boundary text, prepares the boundary field label, selects the nested definition field indent, and returns `_DiagnosticSurfaceExplanationLineSet`.
- `_render_diagnostic_surface_explanation_boundary_line(...)` already exists as the producer for the explanation boundary line request.
...
- The line-set assembly implementation calls `_render_diagnostic_surface_explanation_boundary_line(...)` for the nested `diagnostic_surface_boundary` row rather than constructing the row inline.
- The explanation line-set assembly test now proves that the nested `diagnostic_surface_boundary` row appears in the assembled line set and that line-set assembly delegates nested boundary row production to the explanation boundary line renderer.
...
`test_diagnostic_surface_explanation_line_set_assembly_precedes_human_rendering` now proves that `_assemble_diagnostic_surface_explanation_line_set(...)` includes the rendered nested `diagnostic_surface_boundary` row and delegates that row's production to `_render_diagnostic_surface_explanation_boundary_line(...)`.

`_render_diagnostic_surface_explanation_boundary_line(...)` remains the producer for the explanation boundary line. `_assemble_diagnostic_surface_explanation_line_set(...)` remains responsible only for explanation line-set assembly and inclusion order.
...
`_assemble_diagnostic_surface_explanation_line_set(...)` is the recovered producer for DiagnosticSurface explanation line-set boundary inclusion.
...
The helper carrying the boundary is `_assemble_diagnostic_surface_explanation_line_set(...)`, which consumes the field-specific explanation boundary line renderer rather than owning boundary line text construction.

It does not carry boundary text preparation authority, boundary field-label preparation authority, boundary line-rendering authority, event-ledger authority, cluster mutation authority, schema authority, public CLI authority, or generalized rendering authority.
...
DiagnosticSurface explanation line-set boundary inclusion and DiagnosticSurface explanation boundary line rendering were previously compressed in the explanation line-set assembly test evidence because the test did not prove that the nested `diagnostic_surface_boundary` row is included through the field-specific explanation line renderer.
...
`_assemble_diagnostic_surface_explanation_line_set(...)` owns the recovered explanation line-set boundary inclusion responsibility.
...
- `competency_interrogation_slice_143.md`
