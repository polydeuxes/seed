...
1. **Endpoint visibility is not host ownership.** Prometheus endpoint subjects such as `192.0.2.115:9100` are not silently collapsed into host subjects such as `example_host`.
...
3. **Alias-like knowledge is not ownership transfer.** `prometheus_instance` can preserve the relationship between `example_host` and `192.0.2.115:9100`, but it is not an alias predicate and it does not move endpoint facts to the host.
...
| `up` | Raw Prometheus `instance` endpoint. | Raw provider-specific scrape-target status; canonicalized to endpoint-scoped `availability_status`. | Yes for the scrape endpoint; no for a stable host unless separately asserted. | Tests assert `example_host up` remains unknown while `192.0.2.115:9100 up` is current. |
