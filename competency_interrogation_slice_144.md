# Competency Interrogation Slice 144
...
```text
DiagnosticSurface Explanation Line-Set Consumption Inclusion Request
        !=
DiagnosticSurface Explanation Consumption Line Rendering
```

This slice begins immediately adjacent to Slice 143 in the DiagnosticSurface explanation human line-set assembly path in `seed_runtime/diagnostic_inventory.py`. After the explanation line-set boundary inclusion request became observable, the next adjacent field in the same implementation path is `diagnostic_surface_consumption`. `_assemble_diagnostic_surface_explanation_line_set(...)` includes an already-rendered nested consumption line in the explanation line set, while `_render_diagnostic_surface_explanation_consumption_line(...)` owns concrete consumption line rendering.
...
- `_assemble_diagnostic_surface_explanation_line_set(...)` extracts the explanation consumption, prepares consumption text, prepares the consumption field label, selects the nested definition field indent, and returns `_DiagnosticSurfaceExplanationLineSet`.
- `_render_diagnostic_surface_explanation_consumption_line(...)` already exists as the producer for the explanation consumption line request.
...
- The line-set assembly implementation calls `_render_diagnostic_surface_explanation_consumption_line(...)` for the nested `diagnostic_surface_consumption` row rather than constructing the row inline.
- The explanation line-set assembly test now proves that the nested `diagnostic_surface_consumption` row appears in the assembled line set and that line-set assembly delegates nested consumption row production to the explanation consumption line renderer.
...
The explanation line-set assembly test proved the line-set artifact, heading placement, definition section inclusion, nested CLI flags text inclusion, nested CLI flags renderer delegation, nested description inclusion, nested description renderer delegation, nested JSON support inclusion, nested JSON support renderer delegation, nested record support inclusion, nested record support renderer delegation, nested record scope inclusion, nested record scope renderer delegation, nested diagnostic-surface-boundary inclusion, nested diagnostic-surface-boundary renderer delegation, nested diagnostic-surface-consumption inclusion, dataclass shape, and downstream formatting join. It did not prove that nested `diagnostic_surface_consumption` inclusion in the explanation line set remained separated from explanation consumption line rendering.
...
`test_diagnostic_surface_explanation_line_set_assembly_precedes_human_rendering` now proves that `_assemble_diagnostic_surface_explanation_line_set(...)` includes the rendered nested `diagnostic_surface_consumption` row and delegates that row's production to `_render_diagnostic_surface_explanation_consumption_line(...)`.

`_render_diagnostic_surface_explanation_consumption_line(...)` remains the producer for the explanation consumption line. `_assemble_diagnostic_surface_explanation_line_set(...)` remains responsible only for explanation line-set assembly and inclusion order.
...
`_assemble_diagnostic_surface_explanation_line_set(...)` is the recovered producer for DiagnosticSurface explanation line-set consumption inclusion.
...
The helper carrying the boundary is `_assemble_diagnostic_surface_explanation_line_set(...)`, which consumes the field-specific explanation consumption line renderer rather than owning consumption line text construction.

It does not carry consumption text preparation authority, consumption field-label preparation authority, consumption line-rendering authority, event-ledger authority, cluster mutation authority, schema authority, public CLI authority, or generalized rendering authority.
...
DiagnosticSurface explanation line-set consumption inclusion and DiagnosticSurface explanation consumption line rendering were previously compressed in the explanation line-set assembly test evidence because the test did not prove that the nested `diagnostic_surface_consumption` row is included through the field-specific explanation line renderer.
...
`_assemble_diagnostic_surface_explanation_line_set(...)` owns the recovered explanation line-set consumption inclusion responsibility.
...
- `competency_interrogation_slice_144.md`
...
- broader diagnostic identity, capability, responsibility, inquiry-boundary, explanation, definition, or rendering semantics outside this local boundary line-set inclusion path.
