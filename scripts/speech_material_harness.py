#!/usr/bin/env python3
"""Render text to speech outside Seed, so the result can enter as material.
...
subprocess. Seed neither invokes it nor speaks. What arrives afterwards is a
file, and it enters through the material path like any other body.

The rendering is not a reading of the text. A voice, a speaking rate, and a
...
whose only recoverable relation is that a caller declared the same source text
for both.

Usage:

    speech_material_harness.py --text hello --out hello.wav
    speech_material_harness.py --text hello --out slow.wav --rate 1.6
...
def render(text: str, out: Path, *, voice: str, rate: float) -> None:
    """Write one spoken rendering of this exact text."""
...
def duration_seconds(path: Path) -> float:
    """How long the rendered material runs, read from the file itself."""
...
def write_subtitles(text: str, seconds: float, out: Path) -> None:
    """One cue spanning the whole rendering.

    The cue's extent is the audio's extent. Nothing here knows where within the
    rendering any word falls, and a cue per sentence would assert timings no
...
def write_video(audio: Path, subtitles: Path, out: Path, seconds: float) -> None:
    """A still frame carrying the subtitles, for the length of the audio."""
...
def main() -> int:
    ...
    render(text, args.out, voice=args.voice, rate=args.rate)
