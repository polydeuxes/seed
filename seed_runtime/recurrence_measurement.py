...
class OccurrencePositionFinding:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.source_locality_identity, str) or not (
            ...
            raise RecurrenceMeasurementError("one exact source Locality is required")
        if not isinstance(self.completeness_boundary, EventLedgerBoundary):
            raise RecurrenceMeasurementError("one exact append boundary is required")
        identities = []
...
def get_recorded_occurrence_position_measurement(
    ...
    except (KeyError, TypeError, RecurrenceMeasurementError) as error:
        raise RecurrenceMeasurementError(
