    ...
    def to_json_dict(self) -> dict[str, Any]:
        return {
            "providers": [
                {
                    **asdict(provider),
                    "predicates": list(provider.predicates),
            ...
            "predicates": [
                {
                    ...
                    "provider_count": predicate.provider_count,
                    "providers": list(predicate.providers),
                }
            ...
            "families": [
                {
                    ...
                    "predicate_count": family.predicate_count,
                    "provider_count": family.provider_count,
                    "predicates": list(family.predicates),
...
def build_observation_inventory(
    ...
    *,
    ...
    return ObservationInventory(
        tuple(sorted(providers, key=lambda p: p.name)), predicates, families, metadata
    )


def _providers_from_file(
    path: Path, repo_root: Path
) -> list[ObservationProviderInventory]:
    ...
    for node in tree.body:
        ...
        providers.append(
            ObservationProviderInventory(
                name=_class_string_attr(node, "name")
                or _default_provider_name(node.name),
...
def _is_provider_class(node: ast.ClassDef) -> bool:
    if node.name == "ObservationSource" or any(
        _call_name(base) == "Protocol" for base in node.bases
    ):
        return False
    has_collect = any(
        isinstance(item, ast.FunctionDef) and item.name == "collect"
        for item in node.body
    )
    return has_collect and (
        node.name.endswith("ObservationSource")
        or _class_string_attr(node, "source_type") is not None
    )
...
def _families(
    predicates: tuple[ObservationPredicateInventory, ...],
) -> tuple[ObservationFamilyInventory, ...]:
    grouped: dict[str, dict[str, set[str]]] = defaultdict(
        lambda: {"predicates": set(), "providers": set()}
    )
    ...
    return tuple(
        ObservationFamilyInventory(
            name,
            tuple(sorted(values["predicates"])),
            tuple(sorted(values["providers"])),
        )
...
def format_observation_inventory(inventory: ObservationInventory) -> str:
    ...
    for provider in inventory.providers:
        lines.append(
            f"{provider.name} | {provider.observation_count} | {provider.predicate_count} | {', '.join(provider.predicates[:5])}"
        )
    lines.extend(["", "Predicates", "Predicate | Provider Count | Provider Examples"])
    for predicate in inventory.predicates:
        lines.append(
            f"{predicate.predicate} | {predicate.provider_count} | {', '.join(predicate.providers[:5])}"
        )
    lines.extend(["", "Families", "Family | Predicate Count | Provider Count"])
    for family in inventory.families:
        lines.append(
            f"{family.family} | {family.predicate_count} | {family.provider_count}"
        )
    return "\n".join(lines)
