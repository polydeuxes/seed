...
from seed_runtime.representation_grammar_applicability import RepresentationGrammarApplicabilityProjection
...
def interpret_operator_expression(expression:AttributedOperatorExpression, recovery_projection:RepresentationGrammarRecoveryProjection, recovered_grammar:RecoveredRepresentationGrammar, applicability_projection:RepresentationGrammarApplicabilityProjection, *, interpretation_mechanism_ref:str, invocation_contract_ref:str, lexical_support_refs:tuple[str,...]=(), domain_vocabulary_refs:tuple[str,...]=(), interpretation_convention:str=CONVENTION, supporting_references:tuple[str,...]=(), provenance:tuple[str,...]=(), unknowns:tuple[str,...]=(), conflicts:tuple[str,...]=()):
    ...
    if applicability_projection.recovery_projection_ref != recovery_projection.projection_id: raise OperatorExpressionInterpretationError("grammar applicability does not reference recovery projection")
    if applicability_projection.recovered_grammar_ref != recovered_grammar.grammar_id: raise OperatorExpressionInterpretationError("grammar applicability does not reference recovered grammar")
    if applicability_projection.mechanism_ref != interpretation_mechanism_ref: raise OperatorExpressionInterpretationError("interpretation mechanism mismatch")
    state0=applicability_projection.applicability_state
    ...
    elif state0=="conflict": state,reason,c,alts,unrefs,unsup=("conflict","conflicting applicability evidence is preserved",CandidateInterpretation(),(),(),())
    elif state0!="applicable": state,reason,c,alts,unrefs,unsup=("unknown" if state0=="unknown" else "unsupported",f"grammar applicability is {state0}",CandidateInterpretation(),(),(),())
    ...
    all_conf=_refs((*conflicts,*recovery_projection.conflicts,*recovered_grammar.conflicts,*applicability_projection.conflicts)); all_unk=_refs((*unknowns,*expression.unknowns,*recovery_projection.unknowns,*recovered_grammar.unknowns,*applicability_projection.unknowns))
    if all_conf and state=="interpreted": state="conflict"; reason="conflicting interpretation material is preserved"
    payload={"expression":expression.to_json_dict(),"recovery":recovery_projection.projection_id,"grammar":recovered_grammar.grammar_id,"applicability":applicability_projection.applicability_projection_id,"mechanism":interpretation_mechanism_ref,"contract":invocation_contract_ref,"lexical":sorted(lexical_support_refs),"domain":sorted(domain_vocabulary_refs),"candidate":c.to_json_dict(),"alts":[a.to_json_dict() for a in alts],"unresolved":sorted(unrefs),"unsupported":[s.to_json_dict() for s in unsup],"known_loss":sorted(applicability_projection.known_limitations_or_loss),"unknowns":list(all_unk),"conflicts":list(all_conf),"state":state,"reason":reason,"convention":interpretation_convention}
    pid=_stable("operator-expression-interpretation-projection",payload)
    hand=None
    if state=="interpreted": hand=FutureOperatorAuthorityScopeBindingHandoff(pid,expression.expression_id,expression.operator_ref,expression.workspace_ref,expression.session_ref,state,c.inquiry_or_request_kind,c.relation_or_focus_expressions,c.subject_expressions,c.object_expressions,c.scope_expressions,c.authority_bearing_expressions,c.operator_stated_effect_constraints,c.presentation_preference,c.source_spans,recovered_grammar.grammar_id,applicability_projection.applicability_projection_id,_refs((*lexical_support_refs,*recovered_grammar.lexical_support_refs)),_refs((*provenance,*expression.provenance)),applicability_projection.known_limitations_or_loss,all_unk,all_conf)
    return OperatorExpressionInterpretationProjection("OperatorExpressionInterpretationProjection",pid,expression.expression_id,recovery_projection.projection_id,recovered_grammar.grammar_id,applicability_projection.applicability_projection_id,interpretation_mechanism_ref,invocation_contract_ref,state,reason,c.expression_form,c.inquiry_or_request_kind,c.relation_or_focus_expressions,c.subject_expressions,c.object_expressions,c.scope_expressions,c.requested_movement_class,c.authority_bearing_expressions,c.operator_stated_effect_constraints,c.evidence_support_preference,c.unknown_limitation_preference,c.presentation_preference,c.temporal_expressions,c.source_spans,alts,_refs(lexical_support_refs),_refs(unrefs),unsup,applicability_projection.known_limitations_or_loss,_refs((*supporting_references,*applicability_projection.supporting_references)),_refs((*provenance,*expression.provenance, recovery_projection.projection_id,recovered_grammar.grammar_id,applicability_projection.applicability_projection_id)),all_unk,all_conf,hand)
