...
class ConsumerAudit:
    ...
    def summary(self) -> dict[str, int]:
        return {
            ...
            "single_consumer_items": sum(item.consumer_count == 1 for item in self.items),
...
CONSUMER_PATHS = {
    "projection_builders": ("seed_runtime/state.py", "seed_runtime/projection_store.py"),
    "read_models": (
    ...
    "state_build": ("scripts/seed_local.py", "seed_runtime/state_summary_views.py"),
    "views": ("scripts/seed_local.py", "seed_runtime/state_views.py", "seed_runtime/context_views.py"),
}
...
def build_consumer_audit(
    ...
    *,
    ...
    if diagnostic_filter is None:
        inventory = build_observation_inventory(repo_root, predicate_filter=predicate_filter)
        for predicate in inventory.predicates:
            items.append(_audit_item(predicate.predicate, "observation_predicate", sources))
    if predicate_filter is None:
        ...
        for diagnostic in diagnostics:
            if diagnostic_filter is not None and diagnostic != diagnostic_filter:
                continue
            items.append(_audit_item(diagnostic, "diagnostic", sources))
    return ConsumerAudit(
        ...
        metadata={
            ...
            "consumer_evidence": "; ".join(f"{name}: {', '.join(paths)}" for name, paths in CONSUMER_PATHS.items()),
        },
    )
...
def format_consumer_audit(audit: ConsumerAudit) -> str:
    ...
    for item in audit.items:
        ...
        lines.extend(f"  {consumer}" for consumer in item.consumers) if item.consumers else lines.append("  none")
        lines.extend([
            "",
...
def _audit_item(item: str, kind: str, sources: dict[str, dict[str, str]]) -> ConsumerAuditItem:
    consumers = []
    for consumer, files in sources.items():
        if _mentions_item(files.values(), item):
...
def _read_sources(root: Path) -> dict[str, dict[str, str]]:
    return {consumer: {path: _read(root / path) for path in paths} for consumer, paths in CONSUMER_PATHS.items()}
...
def _mentions_item(sources: Iterable[str], item: str) -> bool:
    needles = {item, json.dumps(item), repr(item), item.replace("_", "-")}
