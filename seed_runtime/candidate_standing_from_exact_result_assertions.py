...
def _source_assertion_references_through(
    ...
    """Apply the exact source rule to one immutable ledger prefix."""
...
def source_assertion_references_for_candidate_standing(
    ...
    """Read every exact source Assertion required by the first source rule."""
...
def _assignment_material(
    *,
    ...
    return {
        ...
        "limits": [
            "one Candidate for each exact source Assertion in event order",
    ...
    *,
    ...
    for recorded in ledger.iter_locality_kind(
        ...
        if (
            ...
            and recorded.material.get("candidate_rule") == CANDIDATE_RULE
        ):
            raise ValueError(
...
def _applicability_act_material(assignment: Event) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence_scope": "exact Candidate Standing source rule",
...
def _candidates(assignment: Event) -> list[dict[str, Any]]:
    return [
        {
            "dimensions": {
                "identity": _candidate_identity(assignment, reference, position),
                "content": {
                    "position": position,
                    "source_assertion_reference": deepcopy(reference),
                    "source_ledger_boundary_identity": assignment.material[
...
def record_complete_candidate_standing(
    ...
    *,
    ...
    """Record the first complete unary Candidate Standing lifecycle."""
