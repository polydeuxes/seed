...
def test_tool_need_history_has_full_cache_hit_and_incremental_empty_shell_parity():
    ledger = EventLedger()
    created = ledger.append(
        ...
        "ws",
        {
            "tool_need": {
                ...
                "workspace_id": "ws",
                "name": "legacy request",
                "summary": "historical request-shaped event",
    ...
    assert full_state.tool_needs == {}
    assert "tool_needs" not in state_to_payload(full_state)

    old_payload = state_to_payload(full_state)
    old_payload["tool_needs"] = {
        "need_legacy": {
            "id": "need_legacy",
            "workspace_id": "ws",
            "name": "legacy request",
    ...
    store.save_snapshot(
        ProjectionSnapshot(
            ...
            last_event_created_at=created.timestamp,
    ...
    cache_hit_state, cache_hit_status = project_state_with_cache(
        ledger, "ws", store, projector=projector
    )
    assert cache_hit_status.cache_hit is True
    assert cache_hit_state.tool_needs == {}

    ledger.append(
        "tool_need.status_changed",
        "ws",
        {"tool_need_id": "need_legacy", "status": "registered"},
    )
    incremental_state, incremental_status = project_state_with_cache(
        ledger, "ws", store, projector=projector
    )
    replayed_state = StateProjector(ledger).project("ws")

    assert incremental_status.incremental_replay is True
    assert incremental_state.tool_needs == replayed_state.tool_needs == {}
