...
def test_capability_inventory_is_read_only():
    ...
    state.tools["tool_web_search"] = _tool("web_search")
...
def test_unverified_capability_inventory_derives_from_missing_facts():
    ...
    state.tools["tool_ssh_access"] = _tool("ssh_access")
...
def test_capability_inventory_sources_keep_contract_metadata_separate_from_admitted_knowledge():
    ...
    state = _project(ledger)
    state.tools["tool_archive_read"] = _tool("archive_read")

    sources = _capability_inventory_sources(state)

    assert sources.executable_operation_contracts.capabilities == {"archive_read"}
    assert sources.admitted_capabilities.capabilities == {"filesystem_read"}
    assert sources.capability_universe() == {"archive_read", "filesystem_read"}
    inventory = build_capability_inventory(state)
    assert [entry.capability for entry in inventory] == [
        "archive_read",
        "filesystem_read",
    ]
    assert [entry.state for entry in inventory] == ["unverified", "verified"]
...
def test_capability_inventory_ordering_is_deterministic():
    ...
    state = _project(ledger)
    state.tools["tool_z"] = _tool("z_capability")
    state.tools["tool_a"] = _tool("a_capability")
...
def test_capability_status_cli_is_read_only_json_and_avoids_runtime(monkeypatch, tmp_path, capsys):
    ...
    state = seed_local.State(workspace_id="local")
    state.tools["tool_web_search"] = _tool("web_search")
    monkeypatch.setattr(seed_local, "projected_state_from_args", lambda _args: state)
