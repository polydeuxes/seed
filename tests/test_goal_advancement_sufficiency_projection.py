...
def _clarification(candidate_resolution, goal, horizon, *, established=(), unknown=(), conflicting=()):
...
def _space(family, candidate_resolution, goal, horizon):
    projection_id = "projection:realization" if family == "operational_realization" else f"projection:{family}"
    return FamilyBoundedCandidateSpace(family, f"space:{family}", candidate_resolution.resolution_id, goal.goal_establishment_id, horizon.horizon_id, projection_id, "snapshot:sufficiency", (f"component:{family}",))


def _testimony(family, candidate_resolution, goal, horizon, *, complete=True, native_projection_id=None, **overrides):
...
def _need_set(candidate_resolution, goal, horizon, clarification=None):
    return assemble_goal_advancement_need_set(
        horizon,
        clarification=clarification if clarification is not None else _clarification(candidate_resolution, goal, horizon),
        inquiry=_inquiry(candidate_resolution, goal, horizon),
        authority=_authority(candidate_resolution, goal, horizon),
        operational_realization=_realization(candidate_resolution, goal, horizon),
    )


def _coverage_set(candidate_resolution, goal, horizon, *, complete=True, families=FAMILIES, testimonies=None):
    return assemble_advancement_need_family_coverage_set(
        ...
        testimonies=tuple(testimonies) if testimonies is not None else tuple(_testimony(f, candidate_resolution, goal, horizon, complete=complete) for f in families),
    )
...
def test_exact_need_set_and_coverage_set_identity_matching_and_binding_conflict():
    ...
    projection = project_goal_advancement_sufficiency(_need_set(candidate_resolution, goal, horizon), _coverage_set(candidate_resolution, goal, other_horizon))
    ...
    coverage = _coverage_set(candidate_resolution, goal, horizon)
    bad_binding = replace(
        coverage,
    ...
    projection = project_goal_advancement_sufficiency(_need_set(candidate_resolution, goal, horizon), bad_binding)
    assert projection.conclusion == "conflicting"
...
def test_material_conflicts_yield_conflicting():
    goal = _goal(); candidate_resolution = _candidate_resolution(goal); horizon = _horizon(candidate_resolution, goal, conflicts=("material horizon conflict",))
    projection = project_goal_advancement_sufficiency(_need_set(candidate_resolution, goal, horizon), _coverage_set(candidate_resolution, goal, horizon))
    assert projection.conclusion == "conflicting"


def test_established_unresolved_need_is_insufficient_and_unknowns_do_not_hide_it():
    goal = _goal(); candidate_resolution = _candidate_resolution(goal); horizon = _horizon(candidate_resolution, goal)
    clarification = _clarification(candidate_resolution, goal, horizon, established=(_item("need:a"),), unknown=(_item("unknown:a", "unknown"),))
    projection = project_goal_advancement_sufficiency(_need_set(candidate_resolution, goal, horizon, clarification), _coverage_set(candidate_resolution, goal, horizon, complete=False))
...
def test_incomplete_or_absent_coverage_without_established_need_is_unknown_and_not_promoted():
    goal = _goal(); candidate_resolution = _candidate_resolution(goal); horizon = _horizon(candidate_resolution, goal)
    projection = project_goal_advancement_sufficiency(_need_set(candidate_resolution, goal, horizon), _coverage_set(candidate_resolution, goal, horizon, complete=False))
...
def test_complete_coverage_alone_and_no_established_need_alone_do_not_establish_sufficiency():
    ...
    complete_coverage = _coverage_set(candidate_resolution, goal, horizon)
    assert project_goal_advancement_sufficiency(absent_need_projection, complete_coverage).conclusion == "unknown"

    supplied_need_set = _need_set(candidate_resolution, goal, horizon)
    partial_coverage = _coverage_set(candidate_resolution, goal, horizon, complete=False)
...
def test_lawful_exclusions_are_neutral_only_with_explicit_non_conflicting_reasons():
    goal = _goal(); candidate_resolution = _candidate_resolution(goal)
    horizon = _horizon(candidate_resolution, goal, explicitly_excluded_need_families=(NeedFamilyExclusion("inquiry", "outside horizon"),))
    need_set = assemble_goal_advancement_need_set(horizon, clarification=_clarification(candidate_resolution, goal, horizon), authority=_authority(candidate_resolution, goal, horizon), operational_realization=_realization(candidate_resolution, goal, horizon))
    coverage = assemble_advancement_need_family_coverage_set(horizon, candidate_spaces=tuple(_space(f, candidate_resolution, goal, horizon) for f in ("clarification", "authority", "operational_realization")), testimonies=tuple(_testimony(f, candidate_resolution, goal, horizon) for f in ("clarification", "authority", "operational_realization")))
...
def test_multiple_established_needs_remain_unordered():
    goal = _goal(); candidate_resolution = _candidate_resolution(goal); horizon = _horizon(candidate_resolution, goal)
    clarification = _clarification(candidate_resolution, goal, horizon, established=(_item("need:a"), _item("need:b")))
    projection = project_goal_advancement_sufficiency(_need_set(candidate_resolution, goal, horizon, clarification), _coverage_set(candidate_resolution, goal, horizon))
...
def test_no_priority_routing_action_authorization_execution_recording_event_ledger_or_mutation():
    goal = _goal(); candidate_resolution = _candidate_resolution(goal); horizon = _horizon(candidate_resolution, goal)
    payload = goal_advancement_sufficiency_projection_json(project_goal_advancement_sufficiency(_need_set(candidate_resolution, goal, horizon), _coverage_set(candidate_resolution, goal, horizon)))
