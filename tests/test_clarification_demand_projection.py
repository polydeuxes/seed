...
from seed_runtime.clarification_demand_projection import OperatorMeaningUncertaintyTestimony, project_clarification_demand
...
def _horizon(goal, **kw):
    base=dict(present_movement_boundary="boundary", evidence_snapshot_refs=(EvidenceSnapshotReference("evidence:1","snapshot:1"),), potentially_relevant_goal_advancement_demand_families=("clarification",)); base.update(kw); return establish_bounded_advancement_horizon(goal, **base)
...
def test_excluded_clarification_family_remains_distinct_and_read_only():
    goal=_goal(); horizon=_horizon(goal, explicitly_excluded_goal_advancement_demand_families=(GoalAdvancementDemandFamilyExclusion("clarification","outside"),)); p=project_clarification_demand(goal,horizon,[_t(goal,horizon)])
