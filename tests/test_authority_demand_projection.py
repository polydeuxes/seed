...
from seed_runtime.authority_demand_projection import AuthorityRequirementTestimony, AuthorityStandingTestimony, project_authority_demand
...
def _horizon(goal): return establish_bounded_advancement_horizon(goal, present_movement_boundary="boundary", evidence_snapshot_refs=(EvidenceSnapshotReference("evidence:1","snapshot:1"),), potentially_relevant_goal_advancement_demand_families=("authority",))
def _req(goal,h,**kw):
...
def test_exact_goal_horizon_evidence_component_authority_class_scope_and_ownership_joins():
    g=_goal(); h=_horizon(g); p=project_authority_demand(g,h,[_req(g,h)],[_auth(g,h)])
    assert len(p.established)==1; assert p.established[0].authority_demand_standing=="established"
    bad=project_authority_demand(g,h,[_req(g,h,testimony_ref="bad-goal",goal_establishment_id="other"),_req(g,h,testimony_ref="bad-scope",applicable_scope_ref="")],[_auth(g,h,testimony_ref="a-bad-goal"),_auth(g,h,testimony_ref="a-bad-scope")])
