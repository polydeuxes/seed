...
def test_from_import_extracts_imported_symbol_object():
    ...
    assert len(facts) == 1
    assert facts[0].relationship_kind == "imports"
    assert "ExampleComponent" in facts[0].object
    assert facts[0].object == "ExampleComponent"
...
def test_definition_observation_does_not_change_import_relationships():
    ...
    assert [fact.object for fact in facts] == ["seed_runtime.events", "ExampleComponent"]
