...
from seed_runtime.operational_realization_demand_projection import OperationalRealizationRequirementTestimony, OperationalRealizationStandingTestimony, project_operational_realization_demand
...
def _horizon(goal): return establish_bounded_advancement_horizon(goal, present_movement_boundary="boundary", evidence_snapshot_refs=(EvidenceSnapshotReference("evidence:1","snapshot:1"),), potentially_relevant_goal_advancement_demand_families=("operational_realization",))
def _req(goal,h,**kw):
...
def test_exact_goal_horizon_evidence_component_transformation_scope_and_ownership_joins():
    g=_goal(); h=_horizon(g); p=project_operational_realization_demand(g,h,[_req(g,h)],[_standing(g,h)])
    assert len(p.established)==1; assert p.established[0].operational_realization_demand_standing=="established"
