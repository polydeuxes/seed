...
from seed_runtime.diagnostic_inventory import (
    ...
    _prepare_diagnostic_surface_definition_boundary_field_label,
...
def test_diagnostic_surface_implementation_reason_line_rendering_precedes_line_set_assembly():
    ...
    reason_value = _prepare_diagnostic_surface_implementation_reason_value(
...
def test_diagnostic_surface_definition_line_set_assembly_precedes_human_rendering():
    ...
    assert "_render_diagnostic_surface_definition_cli_flags_line" in inspect.getsource(
        _assemble_diagnostic_surface_definition_line_set
    )
    assert (
        "  implementation_reason: identity recovered from the diagnostic inventory "
    ...
    assert "  diagnostic_inventory_registration: present" in line_set.lines
    assert (
        "_render_diagnostic_surface_definition_inventory_registration_line"
        in inspect.getsource(_assemble_diagnostic_surface_definition_line_set)
    )
...
def test_diagnostic_surface_explanation_nested_definition_field_indent_selection_precedes_line_rendering():
    field_indent = _select_diagnostic_surface_nested_definition_field_indent()

    assert isinstance(field_indent, _DiagnosticSurfaceNestedDefinitionFieldIndent)
