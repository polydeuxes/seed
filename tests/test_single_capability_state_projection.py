...
def _tool(name="inspect_python", capabilities=None):
    return ToolSpec(name=name, summary="test", input_schema={}, output_schema={}, policy_action=name, implementation="tests:no_execute", toolkit_id="tests", capabilities=capabilities or [])
...
def _ledger(value="verified", expires_at=None):
    ...
    ledger.append("tool.registered", "ws", {"tool": to_plain(_tool(capabilities=["Python Runtime"]))})
...
def test_projection_normalizes_and_exposes_boundaries_without_selection():
    ...
    assert projection.registered_operations[0].name == "inspect_python"
...
def test_json_shape_is_stable_and_typed():
    ...
    assert payload["registered_operations"][0]["boundary"] == "operation_contract_association_only"
