...
from tests.test_bounded_operator_goal_establishment import _goal_admission
...
def test_horizon_preserves_supplied_boundary_scope_and_current_bounds():
    ...
    horizon = establish_bounded_advancement_horizon(
        goal,
        present_movement_boundary="read-only boundary before demand testimony",
        included_scope=("goal lineage", "present boundary", "evidence snapshots"),
        excluded_scope=("demand classification", "execution planning"),
        time_bounds=("current session only",),
        current_state_bounds=("repository snapshot at horizon construction",),
    )
    assert horizon.present_movement_boundary == "read-only boundary before demand testimony"
    assert horizon.included_scope == ("goal lineage", "present boundary", "evidence snapshots")
    assert horizon.excluded_scope == ("demand classification", "execution planning")
