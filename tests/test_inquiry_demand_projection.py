...
def _horizon(goal, **kw):
    base=dict(present_movement_boundary="boundary", evidence_snapshot_refs=(EvidenceSnapshotReference("evidence:1","snapshot:1"),), potentially_relevant_goal_advancement_demand_families=("inquiry",)); base.update(kw); return establish_bounded_advancement_horizon(goal, **base)
...
def test_excluded_inquiry_family_remains_distinct_and_read_only():
    goal=_goal(); horizon=_horizon(goal, explicitly_excluded_goal_advancement_demand_families=(GoalAdvancementDemandFamilyExclusion("inquiry","outside"),)); p=project_inquiry_demand(goal,horizon,[_t(goal,horizon)])
