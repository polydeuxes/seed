...
def test_verification_evidence_does_not_select_execute_or_evaluate_policy(monkeypatch, tmp_path):
    ...
    git.chmod(0o755)
    monkeypatch.setenv("PATH", str(tmp_path))
    monkeypatch.setattr(execution_module.ToolExecutor, "__init__", lambda *a, **k: (_ for _ in ()).throw(AssertionError("ToolExecutor used")))
    monkeypatch.setattr(policy_module.PolicyGate, "evaluate", lambda *a, **k: (_ for _ in ()).throw(AssertionError("policy evaluated")))
    ledger = EventLedger()
    ...
    inspection = build_verification_evidence(state)

    assert inspection.evidence[0].candidate == "git_client"
